...
- 蓝牙魔方连接使用浏览器 Web Bluetooth；需要支持 Web Bluetooth 的浏览器和真实设备。网页会枚举 GATT 服务、订阅 notify/indicate 特征，并在状态栏、蓝牙日志和控制台显示设备发出的原始数据。若设备提供标准 Battery Service，会读取并显示电量；GoCube / Rubik's Connected 会通过 Nordic UART 自动请求状态和电量，并解析其二进制转动包；Giiker / Mi Smart 会解析 20 字节状态包中的最新转动，并向电量服务发送查询。日志也会尝试把通知中的标准转动记号、JSON 转动字段或分隔符转动串解析为转动流，并把转动应用到当前打乱状态上显示是否已复原；计时中检测到已复原会自动停表并保存成绩。成绩详情和导出数据会保留手动/蓝牙停表来源、本把蓝牙转动序列、转动数和 TPS，便于继续接入更多具体品牌协议和排查设备数据。
